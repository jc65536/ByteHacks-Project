...
def recv_message(current_user):
    ...
    received_messages = [
        {'timestamp': message.timestamp, 'sender_email': User.query.filter_by(id=message.sender_id).first().email,
    ...
    sent_messages = [{'timestamp': message.timestamp, 'sender_email': requestor.email,
                      'receiver_email': User.query.filter_by(id=message.recipient_id).first().email,
                      'message': message.message, 'subject': message.subject}
                     for message in
